...
class TransactionLogE2ETest extends AxonBaseE2ETest {
    ...
    def "AccountTransactionsQuery returns list of account's transactions"() {
        ...
        def transactions = [
                new AccountCreditedEvent(accountNumber, 15000, epochMillisAtWarsawOffset("2023-01-10T11:00:00")),
                new AccountDebitedEvent(accountNumber,  3500,  epochMillisAtWarsawOffset("2023-01-11T15:00:00")),
                new AccountDebitedEvent(accountNumber,  2000,  epochMillisAtWarsawOffset("2023-01-20T22:00:00")),
                new AccountCreditedEvent(accountNumber, 15000, epochMillisAtWarsawOffset("2023-02-10T11:00:00")),
                new AccountDebitedEvent(accountNumber,  3500,  epochMillisAtWarsawOffset("2023-02-11T15:00:00")),
                new AccountDebitedEvent(accountNumber,  2000,  epochMillisAtWarsawOffset("2023-02-20T22:00:00"))
        ]
        ...
        with(queryGateway.query(allTransactionsQuery, TransactionsQueryResponse)) {
            it.get().transactions() == [
                    new Transaction(1673344800000, CREDIT, 15000),
                    new Transaction(1673445600000, DEBIT,  3500),
                    new Transaction(1674248400000, DEBIT,  2000),
                    new Transaction(1676023200000, CREDIT, 15000),
                    new Transaction(1676124000000, DEBIT,  3500),
                    new Transaction(1676926800000, DEBIT,  2000)
            ]
        }
        ...
        with(queryGateway.query(lastTwoWeeksQuery, TransactionsQueryResponse)) {
            it.get().transactions() == [new Transaction(1676926800000, DEBIT, 2000)]
        }

        when: 'new transaction is inserted into log'
        def newTransaction = new AccountCreditedEvent(accountNumber, 222, epochMillisAtWarsawOffset("2023-02-16T11:00:00"))
        ...
        with(queryGateway.query(lastTwoWeeksQuery, TransactionsQueryResponse)) {
            ...
            it.get().transactions().contains(new Transaction(1676541600000, CREDIT, 222))
        }
    }

    def epochMillisAtWarsawOffset(String dateTime) {
        LocalDateTime.parse(dateTime).toInstant(krakowOffset).toEpochMilli()
    }
}
